...
namespace crisp
{
    ...
    class NeuralNetwork
    {
        ...
        public:
            ...
            /// @returns matrix holding activation values, each column is one sample, each row one output layer neuron activation value
            ...
            /// @param features: matrix holding one or more samples, each column is one sample, each row one feature. Features should be normalized into [-1, 1]
            /// @param desired_result: matrix holding one or more desired response vectors, each column is one sample, each row is the desired output for that neuron. All values should be either 0 or 1
            /// @param mse_threshold: threshold for mean squared error, recommended range: [0.000001, 0.1]
            ...
            /// @brief compute forward pass for given feature vectors, then compute maximum mean squared error in result
            /// @param features: matrix holding one or more samples, each column is one sample, each row one feature. Features should be normalized into [-1, 1]
            /// @param desired_result: matrix holding one or more desired response vectors, each column is one sample, each row is the desired output for that neuron. All values should be either 0 or 1
            ...
            /// @returns string, used for loading via NeuralNetwork::from_string()
            ...
            /// @brief reset all weights to 1 + eps and all biases to 0 where eps in [-0.25, +0.25] (random, uniformly distributed)
            void clear();
