...
void main()
{
    ...
    float factor_left = project(_reject_factor, _pass_factor, 1.f / (1.f + pow((dist_left*width) / (dist_left*dist_left - center*center), 2*_order)));
    float factor_right = project(_reject_factor, _pass_factor, 1.f / (1.f + pow((dist_right*width) / (dist_right*dist_right - center*center), 2*_order)));
