...
namespace crisp
{
    ...
    GLNativeHandle State<CT>::add_texture(Texture<T, N>& texture)
    {
    ...
    template<typename CT>
    void State<CT>::bind_texture(GLNativeHandle id)
    {
    ...
    template<typename CT>
    GLNativeHandle State<CT>::add_shader(const std::string& path)
    {
        size_t current = 0;
        bool found = false;
    ...
    template<typename CT>
    void State<CT>::bind_shader(GLNativeHandle id)
    {
        _fragment_shader = id;
    }
